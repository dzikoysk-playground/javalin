...
class TestCookieStore {
    ...
    fun `cookieStore works between two handlers`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/cookie-store") { it.cookieStore().set("test-object", 123) }
        app.unsafe.routes.after("/cookie-store") { ctx ->
    ...
    fun `cookieStore can be cleared`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/cookie-storer") { it.cookieStore().set("test-object", 123) }
        app.unsafe.routes.get("/cookie-clearer") { it.cookieStore().clear() }
        app.unsafe.routes.get("/cookie-checker") { it.result("stored: " + it.cookie("javalin-cookie-store")) }
    ...
    fun `cookieStore works between two requests`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/cookie-storer") { it.cookieStore().set("test-object", 123) }
        app.unsafe.routes.get("/cookie-reader") { ctx ->
            if (ctx.cookieStore().get<Any>("test-object") is Int) {
    ...
    fun `cookieStore works between two request with object overwrite`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/cookie-storer") { it.cookieStore().set("test-object", 1) }
        app.unsafe.routes.get("/cookie-overwriter") { it.cookieStore().set("test-object", "Hello world!") }
        app.unsafe.routes.get("/cookie-reader") { ctx ->
    ...
    fun `cookieStore works between two request with multiple objects`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/cookie-storer") { ctx ->
    ...
    fun `cookieStore cookie path is root`() = TestUtil.test { app, http ->
        app.unsafe.routes.get("/") { it.cookieStore().set("s", "Hello world!") }
